...
describe('Arknights/util', () => {
	...
	describe('getAKEventWithDetailTime', () => {
		...
		it('完整流程：列表→详情→解析（单个活动）', async () => {
			mockFetch.mockResolvedValueOnce(mockListResponse([{ cid: 'evt_001', category: 1 }])).mockResolvedValueOnce(mockDetailResponse());
		...
		it('正则不匹配时安全返回空数组（title 有值但 time 为空）', async () => {
			...
			mockFetch.mockResolvedValueOnce(mockListResponse([{ cid: 'evt_001', category: 1 }])).mockResolvedValueOnce(
				mockDetailResponse({
		...
		it('提取 banner（HTML 含 img 标签时，注意无换行）', async () => {
			...
			const contentWithBanner =
				'<div class="act-box"><img src="https://ak.example.com/banner.png"/></div><p><strong>一、材料收集</strong></p><p><strong>活动时间：</strong>2025年12月23日 16:10 - 2026年01月02日 04:10</p>';
			mockFetch
				.mockResolvedValueOnce(mockListResponse([{ cid: 'evt_001', category: 1 }]))
		...
		it('列表请求网络失败时向外抛出', async () => {
			...
			await expect(getAKEventWithDetailTime('https://api.example.com/list', 'https://api.example.com/detail')).rejects.toThrow(
				'Network error',
			);
		});
	});
